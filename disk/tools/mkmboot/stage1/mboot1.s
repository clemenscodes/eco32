;
; mboot1.s -- the master boot, stage 1 (boot code)
;
...
;
; Stage 2 (the boot manager), which is loaded by this
; code, must be in standalone (headerless) executable
; format, stored at absolute disk sectors 34..2047,
...
numsct:
	.word	0			; number of sectors to load
					; 0..2014, must be set (at least)
					; to size of boot manager

start1:
	add	$29,$0,stacktop		; setup stack
	add	$4,$0,strtmsg		; say what is going on
	jal	msgout
	add	$4,$0,34		; start loading with sector 34
	add	$5,$0,loadaddr		; where to load the boot manager
	and	$5,$5,0x3FFFFFFF	; convert to physical address
	ldw	$6,$0,numsct		; get number of sectors to load
	beq	$6,$0,noload		; nothing to load
	jal	rdsct
	ccs				; sync caches
...
nldmsg:
	.byte	"No boot manager", 0x0D, 0x0A, 0
dremsg:
